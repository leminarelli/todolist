...
class App extends Component {
	...
	render() {
		...
		const columns = [
			...
			{ title: 'Feitas', items: items.filter(item => item.status === 'Done'), icon: <CheckIcon /> },
			{ title: 'Todas as tarefas', items, icon: <ListIcon /> },
		];
		return (
			<MuiThemeProvider>
				<div className="App">
					<ConfirmDialog
						...
						message={'Tem certeza que deseja remover todas as tarefas?'}
					...
					<AppBar
						...
						style={{ backgroundColor: 'rgb(139, 215, 224)', position: 'fixed', zIndex: 9999, }}
					/>
					<div className="App-container">
						<div style={{ position: 'fixed', width: '100%', paddingTop: 64, zIndex: 8888, backgroundColor: 'white' }}>
							<TextField
								hintText="Digite a tarefa"
								floatingLabelText="Adicionar tarefa"
								ref={(taskInput) => {
